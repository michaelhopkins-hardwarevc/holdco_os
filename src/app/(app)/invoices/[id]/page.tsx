...
export default async function InvoiceDetailPage({
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground">
            {invoice.clientName ?? "—"}
            {invoice.projectCode ? ` · ${invoice.projectCode} ${invoice.projectName ?? ""}` : ""}
          </p>
        ...
        <div className="flex items-center gap-2">
          {canManage && <PdfButton entityId={active.entityId} invoiceId={invoice.id} />}
        ...
        <TableBody>
          {lines.map((l) => {
            ...
            return (
              <TableRow key={l.id}>
                ...
                <TableCell className="text-right">{formatCents(l.rate)}</TableCell>
                <TableCell className="text-right">{formatCents(l.amount)}</TableCell>
                {isDraft && canManage && (
                  <TableCell className="text-right">
                    {editable ? (
          ...
          {invoice.tax > 0 && (
            <TableRow>
              ...
              <TableCell className="text-right">{formatCents(invoice.tax)}</TableCell>
              {isDraft && canManage && <TableCell />}
            </TableRow>
          )}
          <TableRow>
            ...
            <CardContent>
              <form action={addInvoiceLine} className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Input id="description" name="description" required placeholder="Fixed fee, discount, etc." />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex flex-col gap-2">
                    <Label htmlFor="amount">Amount ($)</Label>
                    <Input id="amount" name="amount" inputMode="decimal" required placeholder="1500.00" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <Label>Type</Label>
      ...
      {isDraft && canManage && lines.some((l) => l.source === "manual" || l.source === "fixed") && (
        <Card>
          <CardHeader>
              ...
              .map((l) => (
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col gap-1">
                    ...
                    <Input name="description" defaultValue={l.description ?? ""} required />
                  </div>
                  <div className="flex w-32 flex-col gap-1">
                    <Label className="text-xs">Amount ($)</Label>
                    <Input name="amount" inputMode="decimal" defaultValue={centsToDollars(l.amount)} required />
                  </div>
                ...
                {payments.length === 0 ? (
                  ...
                  payments.map((p) => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell className="text-right">{formatCents(p.amount)}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
